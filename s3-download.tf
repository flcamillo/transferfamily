...
resource "aws_s3_bucket" "download_bucket" {
  bucket = "${data.aws_caller_identity.current.account_id}-${var.download_bucket_name}"
  tags = {
    Name        = "Uso"
...
resource "aws_s3_bucket_public_access_block" "download_bucket" {
...
resource "aws_s3_bucket_lifecycle_configuration" "download_bucket" {
  bucket = aws_s3_bucket.download_bucket.id
  rule {
    id = "remove_old_files"
    filter {
...
data "aws_iam_policy_document" "download_bucket_policy_document" {
  statement {
...
resource "aws_s3_bucket_policy" "download_bucket_policy" {
  bucket = aws_s3_bucket.download_bucket.id
  policy = data.aws_iam_policy_document.download_bucket_policy_document.json
}
